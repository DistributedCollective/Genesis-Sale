...
import "./CFISHToken.sol";

contract CrowdSaleFish is Ownable {
    ...
    constructor(
        ...
        tokenTotalSupply = CFISHToken(token).totalSupply();
    ...
    function start(
        ...
        CFISHToken tokenInstance = CFISHToken(token);
        require(
    ...
    function withdrawTokens() external onlyOwner() saleDone() {
        ...
        CFISHToken tokenInstance = CFISHToken(token);
        //tokenInstance.transfer(sovrynAddress, tokensFish);
        require(
            tokenInstance.transfer(sovrynAddress, tokensFish),
            "transfer failed"
        );
